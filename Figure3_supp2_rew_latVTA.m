...
% reward files
fn1 = 'm75_milknocue.csv';
fn2 = 'm725_milknocue.csv';
fn3 = 'm723_freemilknocue2.csv';

% shock files
fn_sh1 = 'gcamp_m75_04.csv';
fn_sh2 = 'gcamp_m725_04.csv';
fn_sh3 = 'gcamp_m723_04.csv';
...
gc_sh1 = xlsread(fn_sh1);  gc_sh1(166900:end,4) = NaN;  % get rid of signal cut at end
gc_sh2 = xlsread(fn_sh2); 
gc_sh3 = xlsread(fn_sh3); 

% extract dF/F, rew_ttl, lick1_ttl, shock_ttl
[gc_dff1, rew_ttl1, lick_ttl1, lick_ttl1_all] = extract_gc_rew_lick_ttl(gc1);
[gc_dff2, rew_ttl2, lick_ttl2, lick_ttl2_all] = extract_gc_rew_lick_ttl(gc2);
[gc_dff3, rew_ttl3, lick_ttl3, lick_ttl3_all] = extract_gc_rew_lick_ttl(gc3);

[gc_dff1_sh, sh_ttl1] = extract_gc_sh_ttl(gc_sh1);
[gc_dff2_sh, sh_ttl2] = extract_gc_sh_ttl(gc_sh2);
[gc_dff3_sh, sh_ttl3] = extract_gc_sh_ttl(gc_sh3);

%%
% z-score: 
[gc_z_rew1, gc_z_sh1] = zscore(gc_dff1, gc_dff1_sh);
[gc_z_rew2, gc_z_sh2] = zscore(gc_dff2, gc_dff2_sh);
[gc_z_rew3, gc_z_sh3] = zscore(gc_dff3, gc_dff3_sh);

%% align with reward
gc_lick1 = align_gcampttl(gc_z_rew1, lick_ttl1, 5*Fs_gcamp, 5*Fs_gcamp);
gc_lick2 = align_gcampttl(gc_z_rew2, lick_ttl2, 5*Fs_gcamp, 5*Fs_gcamp);
gc_lick3 = align_gcampttl(gc_z_rew3, lick_ttl3, 5*Fs_gcamp, 5*Fs_gcamp);

gc_lick_comb = [gc_lick1; gc_lick2; gc_lick3];

clear gc_lick_comb2
gc_lick_comb2(1,:,:) = gc_lick1;
gc_lick_comb2(2,1:10,:) = gc_lick2;
gc_lick_comb2(3,1:5,:) = gc_lick3;

%% align with shock 
gc_sh1 = align_gcampttl(gc_z_sh1, sh_ttl1, 5*Fs_gcamp_sh, 5*Fs_gcamp_sh); gc_sh1 = gc_sh1(1:5,:);
gc_sh2 = align_gcampttl(gc_z_sh2, sh_ttl2, 5*Fs_gcamp_sh, 5*Fs_gcamp_sh); gc_sh2 = gc_sh2(1:5,:);
gc_sh3 = align_gcampttl(gc_z_sh3, sh_ttl3, 5*Fs_gcamp_sh, 5*Fs_gcamp_sh); gc_sh3 = gc_sh3(1:5,:);

gc_sh_comb = [gc_sh1; gc_sh2; gc_sh3];

gc_sh_comb2(1,:,:) = gc_sh1;
gc_sh_comb2(2,:,:) = gc_sh2;
gc_sh_comb2(3,:,:) = gc_sh3;


%% shuffled data - reward
[gc_lick_comb_shuff, gc_sh_comb_shuff] = shuffle_multiplemice(gc_dff1,gc_dff2, gc_dff3,gc_dff1_sh,gc_dff2_sh,gc_dff3_sh, ...
    lick_ttl1, lick_ttl2, lick_ttl3, sh_ttl1, sh_ttl2, sh_ttl3, ...
...
for j = 1:n_shuffles
    disp(j)
    [gc_rew_shuffle_aligned(j,:,:), gc_sh_shuffle_aligned(j,:,:)]= shuffle_multiplemice(gc_dff1,gc_dff2, gc_dff3,gc_dff1_sh,gc_dff2_sh,gc_dff3_sh, ...
    lick_ttl1, lick_ttl2, lick_ttl3, sh_ttl1, sh_ttl2, sh_ttl3, ...
    Fs_gcamp, Fs_gcamp_sh);
end
...
percentile_ = .01/X;  % .01/X Change this value for reward or shock condition
...
%gc_lick_comb2 = gc_lick_comb2(1:2,:,:);
...
plot(squeeze(nanmean(nanmean(gc_lick_comb2))), 'color', darkdarkblue)
...
imagesc(gc_lick1), colormap(b2b), colorbar, caxis([-5 5]), hold on
...
plot(squeeze(nanmean(nanmean(gc_sh_comb2))), 'color', darkdarkblue)
plot([500 500], [-2 4], '--', 'color', [.7 .7 .7])
...
imagesc(gc_sh1), colormap(b2b), colorbar, caxis([-5 5]), hold on
%plot([125 125], [1 8], '--', 'color', 'g')
set(gca, 'xtick', 0:Fs_gcamp_sh:1000, 'xticklabel', -5:5)
...
%%
function [gc_lick_comb, gc_sh_comb] = shuffle_multiplemice(gc_dff1,gc_dff2, gc_dff3,gc_dff1_sh,gc_dff2_sh,gc_dff3_sh, ...
    lick_ttl1, lick_ttl2, lick_ttl3, sh_ttl1, sh_ttl2, sh_ttl3, ...
    Fs_gcamp, Fs_gcamp_sh)

    % shuffle data
    gc_dff1 = shuffle(gc_dff2, 1);
    gc_dff2 = shuffle(gc_dff2, 1);
    gc_dff3 = shuffle(gc_dff3, 1);
    gc_dff1_sh = shuffle(gc_dff1_sh, 1);
    gc_dff2_sh = shuffle(gc_dff2_sh, 1);
